...
async function scrape(task,book){let tab;try{tab=await chrome.tabs.create({url:task.url,active:false});if(tab.status!=="complete")await loaded(tab.id);await wait(1800);let response;for(let attempt=0;attempt<3;attempt++){try{response=await chrome.tabs.sendMessage(tab.id,{type:"PREZZOLIBRI_SCRAPE",platform:task.platform,sold:Boolean(task.sold),isbn:book.isbn,title:book.title,authors:book.authors});break;}catch{await wait(700);}}return response?.listings||[];}catch{return [];}finally{if(tab?.id)await chrome.tabs.remove(tab.id).catch(()=>{});}}
