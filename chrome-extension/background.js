...
chrome.runtime.onConnect.addListener(port=>{
  ...
  port.onMessage.addListener(async message=>{
    ...
    for(let i=0;i<work.length;i++){
      ...
      const listings=work[i].fallback?packet.listings.filter(item=>relevantToBook(item,message.book)).map(item=>({...item,relevance:item.relevance==="exact"?"high":item.relevance})):packet.listings;
      all.push(...listings);
    ...
    const platforms=["vinted","ebay","abebooks","subito","amazon"],extensionVersion=chrome.runtime.getManifest().version;
