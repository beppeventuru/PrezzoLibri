...
function renderWorkspace() {
  ...
  $("#comparableList").innerHTML = b.comparables.length ? b.comparables.map(c => `<article class="comparable"><div><b>${c.platform}</b><span>${c.evidence_type === "sold" ? "VENDUTO" : "ATTIVO"}</span><h3>${c.title || "Confronto senza titolo"}</h3><small>${c.relevance.replace("exact","ISBN esatto")} · ${new Date(c.observed_at).toLocaleDateString("it-IT")}</small></div><strong>${euro(c.price + c.shipping)}</strong>${c.url ? `<a href="${c.url}" target="_blank" rel="noopener">Apri ↗</a>`:""}</article>`).join("") : `<p class="empty">Nessun confronto: apri i marketplace e aggiungi i risultati pertinenti.</p>`;
}

function renderMarketplaceResults(results) {
  const names = { vinted:"Vinted", ebay:"eBay", abebooks:"AbeBooks", subito:"Subito", amazon:"Amazon" };
  $("#marketResults").innerHTML = results.map(result => `<article class="market-result"><div class="market-result-head"><h3>${escapeHtml(names[result.platform] || result.platform)}</h3><span class="${escapeHtml(result.status)}">${result.listings.length ? `${result.listings.length} risultati` : result.status === "blocked" ? "Non accessibile" : "Nessun risultato"}</span></div>${result.listings.length ? result.listings.map(item => `<div class="listing"><div><b>${escapeHtml(item.title || "Offerta")}</b><small>${item.relevance === "exact" ? "ISBN esatto" : item.relevance === "high" ? "Stessa edizione probabile" : "Da verificare"}${item.condition ? ` · ${escapeHtml(item.condition)}` : ""}</small></div><strong>${euro(item.price + item.shipping)}</strong><a href="${escapeHtml(item.url)}" target="_blank" rel="noopener">Verifica ↗</a></div>`).join("") : `<p class="empty">${escapeHtml(result.note || "Nessuna offerta verificabile trovata.")}</p>`}</article>`).join("");
}
