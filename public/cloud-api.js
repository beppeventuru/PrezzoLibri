...
async function cloudRequest(path, options={}) {
  ...
  const importMatch=path.match(/^\/api\/books\/(\d+)\/import-marketplaces$/);if(importMatch&&method==="POST"){const bookId=Number(importMatch[1]);const allowed={vinted:["www.vinted.it","vinted.it"],ebay:["www.ebay.it","ebay.it"],abebooks:["www.abebooks.it","abebooks.it"],subito:["www.subito.it","subito.it"],amazon:["www.amazon.it","amazon.it"]};const candidates=(input.results||[]).flatMap(result=>(result.listings||[]).map(item=>({...item,platform:result.platform}))).filter(item=>{try{return allowed[item.platform]?.includes(new URL(item.url).hostname)&&Number(item.price)>0&&Number(item.price)<100000}catch{return false}});if(candidates.some(item=>item.platform==="amazon"&&/^Usato\s*-/i.test(item.condition||""))){const {error}=await db.from("comparables").delete().eq("book_id",bookId).eq("platform","amazon").ilike("title","%offerta usata più economica%");if(error)throw error;}const urls=[...new Set(candidates.map(item=>item.url))];let existing=[];if(urls.length){const {data,error}=await db.from("comparables").select("url").eq("book_id",bookId).in("url",urls);if(error)throw error;existing=(data||[]).map(row=>row.url);}const rows=candidates.filter((item,index)=>!existing.includes(item.url)&&candidates.findIndex(other=>other.url===item.url)===index).map(item=>({book_id:bookId,platform:item.platform,url:item.url,title:item.title||"",price:Number(item.price),shipping:Math.max(0,Number(item.shipping)||0),condition:item.condition||"",relevance:["exact","high","medium","low"].includes(item.relevance)?item.relevance:"medium",evidence_type:item.evidenceType==="sold"?"sold":"active",accepted:true}));if(rows.length){const {error}=await db.from("comparables").insert(rows);if(error)throw error;}return{added:rows.length};}
